...
function RiderRatingsPage() {
    ...
    useEffect(() => {
        const fetchTrips = async () => {
            try {
                ...
                const tripsWithRatings = tripsArray.filter(trip => trip.customer_rating !== null);
    ...
    const exportToCSV = () => {
        ...
        link.setAttribute("download", "rider_ratings_export.csv");
    ...
    const filteredTrips = sortedTrips.filter((trip) => {
        ...
        const driverRating = trip.customer_rating || 0;
        const matchesFilter =
            filterRating === "all" ||
...
const ratingStats = useMemo(() => {
  ...
  const ratings = filteredTrips
    .map(trip => Number(trip.customer_rating)) // force to number
        ...
        <div className={`ratings-dashboard ${isDark ? 'dark-theme' : ''}`}>
            ...
            <div className="ratings-table-container">
                <table className="ratings-table">
                    <thead>
                        <tr>
                            <th onClick={() => requestSort('mainName')}>
                                Rider {sortConfig.key === 'mainName' && (sortConfig.direction === 'ascending' ? '↑' : '↓')}
                            </th>
                            <th onClick={() => requestSort('driverName')}>
                                Driver {sortConfig.key === 'driverName' && (sortConfig.direction === 'ascending' ? '↑' : '↓')}
                            </th>
                            <th onClick={() => requestSort('customer_rating')}>
                                Rating {sortConfig.key === 'customer_rating' && (sortConfig.direction === 'ascending' ? '↑' : '↓')}
                            </th>
                            <th>Trip Details</th>
                                ...
                                <tr key={trip.id} className="rating-row">
                                    <td>
                                        <div className="driver-info">
                                            <div className="avatar-sm">
                                                {trip.mainName ? trip.mainName.charAt(0) : "U"}
                                            </div>
                                            <div>
                                                <div className="name">{trip.mainName || `User ${trip.customerId}`}</div>
                                    ...
                                    <td>
                                        <div className="rating-display">
                                            <div className="stars">
                                                {renderStars(trip.customer_rating)}
                                            </div>
                                            <span className="rating-value">{Number(trip.customer_rating || 0).toFixed(1)}</span>
                                        </div>
                                    </td>
                                    <td>
                                ...
                                <div className="modal-section">
                                    ...
                                    <div className="info-grid">
                                        <div className="info-item">
                                            <label>Rider</label>
                                            <span>{currentTrip.mainName || `User ${currentTrip.customerId}`}</span>
                                        </div>
                                        <div className="info-item">
                                            <label>Driver</label>
                                    ...
                                    <div className="info-grid">
                                        ...
                                        {currentTrip.pickupTime && (
                                            <div className="info-item">
                                                ...
                                                <span>{new Date(currentTrip.pickupTime).toLocaleTimeString()}</span>
                                            </div>
                                        )}
                                        {currentTrip.dropOffTime && (
                                            <div className="info-item">
                                                <label>Dropoff Time</label>
                                                <span>{new Date(currentTrip.dropOffTime).toLocaleTimeString()}</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="modal-section full-width">
                                    ...
                                    <div className="rating-display-large">
                                        <div className="stars-large">
                                            {renderStars(currentTrip.customer_rating, 'large')}
                                            <span className="rating-value-large">{Number(currentTrip.customer_rating || 0).toFixed(1)}</span>
                                        </div>
                                ...
                                {currentTrip.cancellation_reason && (
                                    <div className="modal-section full-width">
                                        ...
                                        <div className="info-grid">
                                            <div className="info-item">
                                                <label>Reason</label>
                                                <span>{currentTrip.cancellation_reason}</span>
                                            </div>
                                            {currentTrip.cancel_by && (
                                                <div className="info-item">
                                                    <label>Canceled By</label>
                                                    <span>{currentTrip.cancel_by}</span>
                                                </div>
                                            )}
                                        </div>
                                    </div>
