...
import "./ScheduledRides.css"; // Reuse the same CSS
import { 
  FaDownload, 
  FaMapMarkerAlt, 
  FaMoneyBillWave, 
...
function ScheduledRides() {
  ...
  const [filters, setFilters] = useState({
    status: "pending", // Default to pending status only
    sortBy: "requestDate",
    sortOrder: "asc" // Show upcoming rides first
  });
  const [showFilters, setShowFilters] = useState(false);
  const limit = 10;

  // Fetch rides with pending status only
  ...
  // Filter rides based on search term (only pending rides are shown)
  ...
  const clearFilters = () => {
    setFilters({ status: "pending", sortBy: "requestDate", sortOrder: "asc" });
  };
  ...
  const exportCSV = () => {
    ...
    link.setAttribute("download", "scheduled_rides.csv");
  ...
  return (
    <div className="admin-rides-page">
      ...
      <div className="info-banner">
        ...
        <p>Showing all upcoming rides with pending status</p>
      </div>
      ...
      <div className="controls-container">
        <div className="search-box">
          ...
          <input
            type="text"
            placeholder="Search scheduled rides by customer, driver, or location..."
            value={searchTerm}
      ...
      <div className="rides-grid">
        {filteredRides.length > 0 ? filteredRides.map((ride) => (
          <div className="ride-card scheduled-card" key={ride.trip_id}>
            ...
            <div className="card-content">
              ...
              <div className="info-row">
                ...
                <div className="info-text">
                  ...
                  <span className="value">{formatTime(ride.pickupTime)}</span>
                </div>
              </div>
          ...
          <div className="empty-state">
            ...
            <p>There are currently no upcoming rides with pending status</p>
          </div>
        )}
      </div>
        ...
        <div className="modal-overlay" onClick={() => setSelectedRide(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Scheduled Trip Details #{selectedRide.trip_id}</h2>
            ...
            <div className="modal-body">
              <div className="detail-grid">
                <div className="detail-group">
                  ...
                  <DetailItem label="Driver ID" value={selectedRide.driverId || "Not assigned yet"} />
                  <DetailItem label="Vehicle Type" value={selectedRide.vehicle_type || "Not specified"} />
                  <DetailItem label="Status" value="Scheduled (Pending)" badgeClass="status-pending" />
                </div>

                <div className="detail-group">
                  <h3>Schedule</h3>
                  <DetailItem label="Request Date" value={formatDate(selectedRide.requestDate)} />
                  <DetailItem label="Scheduled Pickup" value={formatDate(selectedRide.pickupTime)} />
                  <DetailItem label="Estimated Dropoff" value={formatDate(selectedRide.dropOffTime)} />
                </div>
                ...
                <div className="detail-group">
                  ...
                  <DetailItem label="Estimated Distance" value={selectedRide.distance_traveled ? `${selectedRide.distance_traveled} km` : "Not available"} />
                  <DetailItem label="Estimated Duration" value={selectedRide.duration_minutes ? `${selectedRide.duration_minutes} min` : "Not available"} />
                </div>

                <div className="detail-group">
                  <h3>Payment</h3>
                  <DetailItem label="Estimated Fare" value={selectedRide.fare ? `$${selectedRide.fare}` : "Not set"} />
                  <DetailItem label="Payment Method" value={selectedRide.payment_method || "Not specified"} />
                </div>
              </div>
            </div>
          </div>
